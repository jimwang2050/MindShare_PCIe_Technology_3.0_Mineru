...
## 4.1 Prefetchable vs. Non-prefetchable Memory Space | 4.1 可预取内存空间与不可预取内存空间
...
## 4.4 BAR Example 2: 64-bit Memory Address Space Request | 4.4 BAR 示例 2: 64 位存储器地址空间请求
...
## 4.5 BAR Example 3: IO Address Space Request | 4.5 BAR 示例 3：IO 地址空间请求
...
## 4.6 All BARs Must Be Evaluated Sequentially | 4.6 所有BAR必须按顺序评估
...
## 4.7 Resizable BARs | 4.7 可调整大小的 BAR
...
## 4.8 Base and Limit Registers | 4.8 基址与界限寄存器
...
## 4.10 Prefetchable Range (P-MMIO) | 4.10 可预取范围 (P-MMIO)
...
## 4.11 Non-Prefetchable Range (NP-MMIO) | 4.11 不可预取范围 (NP-MMIO)
...
## 4.13 Unused Base and Limit Registers | 4.13 未使用基址和限制寄存器
...
## 4.14 Sanity Check: Registers Used For Address Routing | 4.14 正确性检查：用于地址路由的寄存器
...
## 4.15 TLP Routing Basics | 4.15 TLP 路由基础
...
## 4.16 Receivers Check For Three Types of Traffic | 4.16 接收器检查三种类型的流量
...
## 4.17 Routing Elements | 4.17 路由元素
...
## 4.18 Three Methods of TLP Routing | 4.18 TLP 路由的三种方法

## 4.19 General | 4.19 概述
...
## 4.20 Purpose of Implicit Routing and Messages | 4.20 隐式路由和报文的目的
...
## 4.21 Split Transaction Protocol | 4.21 拆分事务协议
...
## 4.22 Posted versus Non-Posted | 4.22 Posted 与非 Posted 事务
...
## 4.24 TLP Header Overview | 4.24 TLP 头部概述
...
## 4.25 Applying Routing Mechanisms | 4.25 应用路由机制
...
## 4.27 Key TLP Header Fields in ID Routing | 4.27 ID 路由中的关键 TLP 头部字段
...
## 4.28 Endpoints: One Check | 4.28 端点：单一检查
...
## 4.29 Address Routing | 4.29 地址路由
...
## 4.30 Key TLP Header Fields in Address Routing | 4.30 地址路由中的关键 TLP 头标字段
...
## 4.31 Endpoint Address Checking | 4.31 端点地址检查
...
## 4.32 Switch Routing | 4.32 交换机路由
...
## 4.33 Downstream Traveling TLPs (Received on Primary Interface) | 4.33 下游传输的TLP（在主接口上接收）
...
## 4.34 Upstream Traveling TLPs (Received on Secondary Interface) | 4.34 向上游传输的 TLP（在次级接口上接收）
...
## 4.35 Multicast Capabilities | 4.35 组播能力
...
## 4.36 Implicit Routing | 4.36 隐式路由
...
## 4.37 Key TLP Header Fields in Implicit Routing | 4.37 隐式路由中的关键 TLP 头字段
...
## 4.38 Message Type Field Summary | 4.38 消息类型字段总结
...
## 4.39 Endpoint Handling | 4.39 端点处理
...
## 4.40 Switch Handling | 4.40 交换机处理
...
## 4.41 PCI Express Technology | 4.41 PCI Express 技术
...
## 4.42 5 TLP Elements | 4.42 5 TLP 元素
...
## 4.43 Introduction to Packet-Based Protocol | 4.43 基于数据包协议的介绍

## 4.44 General | 4.44 概述
...
## 4.45 Motivation for a Packet-Based Protocol | 4.45 采用基于数据包协议的动机
...
## 4.46 Packet Formats Are Well Defined | 4.46 数据包格式定义明确
...
## 4.47 Framing Symbols Define Packet Boundaries | 4.47 成帧符号定义包边界
...
## 4.48 CRC Protects Entire Packet | 4.48 CRC 保护整个包
...
## 4.49 TLP Assembly And Disassembly | 4.49 TLP 组装与拆解
...
## 4.52 TLP Structure | 4.52 TLP 结构
...
## 4.53 Generic TLP Header Format | 4.53 通用 TLP 头格式

## 4.54 General | 4.54 概述
...
## 4.55 Generic Header Field Summary | 4.55 通用头字段汇总
...
## 4.56 Header Type | 4.56 Format Field Encodings
...
## 4.58 Using Byte Enables | 4.58 使用字节使能
...
## 4.59 Byte Enable Rules | 4.59 字节使能规则
...
## 4.60 Transaction Descriptor Fields | 4.60 事务描述符字段
...
## 4.61 Additional Rules For TLPs With Data Payloads | 4.61 带有数据载荷的TLP的附加规则
...
## 4.63 Memory Requests | 4.63 存储器请求
...
## 4.64 PCI Express Technology | 4.64 PCI Express 技术
...
## 4.65 Configuration Requests | 4.65 配置请求
